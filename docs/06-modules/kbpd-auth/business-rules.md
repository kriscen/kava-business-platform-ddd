...
客户端 → /oauth2/authorize?client_id=xxx
  ...
  → TenantAwareAuthenticationFilter（用 clientId 查询 RegisteredClientRepository，从 ClientSettings 提取服务端可信的 tenantId、userType）
  → ExtendAuthenticationToken（携带服务端可信的 tenantId、userType）
  → CustomerAuthenticationProvider → PwdUserDetailsService（按 userType 路由）
```

### 约束

- `client_id` 必须在系统中注册，否则抛出 `AuthenticationServiceException`
- `tenantId` 和 `userType` 从 OAuth2 Client 的 ClientSettings 中获取（服务端可信数据源），不信任前端表单提交的值
- 前端篡改 tenantId/userType 不影响认证结果，认证使用 ClientSettings 中的服务端值
...
- `DBRegisteredClientRepository.save()` 不支持（客户端数据由 UPMS 管理）
- `DBRegisteredClientRepository.findById()` 委托给 `findByClientId()`（内部 id 等于 clientId）

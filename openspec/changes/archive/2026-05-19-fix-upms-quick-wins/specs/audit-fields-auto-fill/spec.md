...
- **THEN** 系统 SHALL 自动将 creator 和 modifier 设为当前认证用户的标识（用户 ID），gmtCreate 和 gmtModified 设为当前时间，delFlag 设为 "0"

#### Scenario: 更新记录时自动填充审计字段

- **WHEN** 通过 MyBatis-Plus 更新一条记录，且 PO 实体的 modifier、gmtModified 字段标注了填充注解
- **THEN** 系统 SHALL 自动将 modifier 设为当前认证用户的标识（用户 ID），gmtModified 设为当前时间

...
用户详情和列表响应 SHALL 包含以下关联名称字段：`deptName`（部门名称）、`tenantName`（租户名称）。用户详情响应 SHALL 额外包含 `roleNames`（角色名称列表）。

#### Scenario: 用户详情返回关联名称
- **WHEN** 前端调用 `GET /api/v1/sys/user/{id}`
- **THEN** 响应包含 `deptName`、`tenantName`、`roleNames` 字段
- **AND** 当关联 ID 存在但对应记录已删除时，名称字段返回 null

#### Scenario: 用户列表返回部门名称和租户名称
- **WHEN** 前端调用 `GET /api/v1/sys/user/page`
- **THEN** 每条记录包含 `deptName`、`tenantName` 字段
...
### Requirement: 部门响应补充父级名称
部门详情和列表响应 SHALL 包含 `parentName` 字段，用于前端展示部门层级路径。

#### Scenario: 部门详情返回父级名称
- **WHEN** 前端调用 `GET /api/v1/sys/dept/{id}` 且该部门有父级
- **THEN** 响应包含 `parentName` 字段，值为父级部门名称

#### Scenario: 部门详情为顶级部门
- **WHEN** 前端调用 `GET /api/v1/sys/dept/{id}` 且该部门 pid 为 null

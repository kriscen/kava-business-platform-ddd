...
系统 SHALL 支持为租户订阅 App，通过写入 `sys_tenant_app` 关系记录实现。订阅后租户自动获得该 App 包含的菜单能力。前端对接文档 SHALL 包含 `POST /api/v1/sys/tenant-app/subscribe` 端点定义，请求体为 `SysTenantAppRequest`（tenantId、appId），返回 `JsonResult<Void>`。
...
系统 SHALL 支持租户退订 App，退订后租户失去该 App 的菜单能力。前端对接文档 SHALL 包含 `POST /api/v1/sys/tenant-app/unsubscribe` 端点定义，请求体为 `SysTenantAppRequest`（tenantId、appId），返回 `JsonResult<Void>`。

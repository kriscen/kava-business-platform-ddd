...
### Requirement: 部门树下拉端点
系统 SHALL 提供 `GET /api/v1/sys/dept/tree` 端点，返回当前租户下所有部门的树形结构（id、name、pid、children），用于用户表单的部门单选。

#### Scenario: 查询部门树
- **WHEN** 前端调用 `GET /api/v1/sys/dept/tree`
- **THEN** 返回当前租户下所有部门的树形结构
- **AND** 每个节点包含 id、name、pid、children

#### Scenario: 当前租户无部门
- **WHEN** 前端调用 `GET /api/v1/sys/dept/tree` 且当前租户无部门
- **THEN** 返回空列表
